...
using DataFrames, CSV, Plots, DifferentialEquations, Optimization, DiffEqParamEstim,OptimizationOptimJL, PlutoUI, Dates

# ╔═╡ 9c2b668b-97fe-4eb1-9fce-e9b7c6ca76d1
theme(:dracula)
...
df= CSV.read("/home/joaquinm/Labo Model. Cont/TP1/WHO-COVID-19-global-data.csv",DataFrame)

# ╔═╡ be179332-f80d-469e-b436-aaac1a41dc3f
begin
	df_arg0 = filter(row -> row.Country == "Argentina", df)
	df_arg = df_arg0[58:7:1336, [1,6,8]]
end

# ╔═╡ ce9c13e5-6d84-458a-aa6d-476968c0587e
begin
	infectados_sem = prepend!(diff(df_arg[!,2]),0)/45_000_000
	muertos_sem = prepend!(diff(df_arg[!,3]),0)/45_000_000
end

# ╔═╡ a33ffb13-517e-4179-8de5-5f6d9af3328c
begin
	tf_ola1 = 42
	tf_ola2 = 52
end

# ╔═╡ 75abc9c5-3f6b-4326-bba2-b50937d95348
begin
	ola1 = infectados_sem[1:tf_ola1]
	ola1y2 = infectados_sem[1:tf_ola2]
end

# ╔═╡ 9772e01f-9849-4fcb-a5c5-801db5c7391d
begin
	#puntos criticos
	olas_x = [tf_ola1,tf_ola2]
	olas_y1 = [infectados_sem[tf_ola1],infectados_sem[tf_ola2]] #infectados
	olas_y2 = [muertos_sem[tf_ola1],muertos_sem[tf_ola2]]#muertes
end

# ╔═╡ 9eb213db-6436-4d04-a1a3-5df4fc12de34
begin
plot(infectados_sem[1:tf_ola2+3],label = "y")
plot!(ola1y2, label = "2da ola")
plot!(ola1 , label = "1ra ola")
scatter!(olas_x,olas_y1, color = "red", label = "Puntos criticos")
title!("Infectados por semana")
xlabel!("Semana")
ylabel!("Casos")
end
...
# ╔═╡ fc12a935-8f63-4c98-8eed-46e3d260f722
begin
	t_1 = collect((1:tf_ola1))
	t_2 = collect((1:tf_ola2))
end

# ╔═╡ 75905ce1-5e03-405f-b42e-7dfc4b5eea11
function SIR(du, u, p, t)
	b, s = p
	
	du[1] = -b*u[1]*u[2]
	
	du[2] = b*u[1]*u[2] - s*u[2]
	
	du[3] = s*u[2]
end

# ╔═╡ 7e75c214-7b5c-4f22-aa0f-b210d45ea88c
ϵ = 0.01

# ╔═╡ c323b75e-1b4d-48de-a38b-05786269b164
@bind β Slider(0:0.1:1)

# ╔═╡ f28d8284-3045-4d66-9c7f-92b352fc4880
@bind σ Slider(0:0.1:1)

# ╔═╡ a2d56420-3ac8-4c42-bf4c-701750239674
begin
	SIR_problem    = ODEProblem(SIR,[1 - ϵ, ϵ, 0.],(1,tf_ola1),abstol=1e-14,reltol=1e-10)
	SIR_dummie_solution = solve(SIR_problem, AutoTsit5(Rosenbrock23()), p = [β, σ], maxiters=40000)
end

# ╔═╡ d8d0243a-bd5e-44e4-95ea-044d0fc80ab5
md""" Para el gráfico que viene a continuación mover los sliders de los parametros de arriba para ver como varía """

# ╔═╡ 6b2a0ad4-631f-4866-b408-9e7ce6fc21fd
begin
plot(SIR_dummie_solution.(t_1, idxs=1) .* SIR_dummie_solution.(t_1, idxs=2) .* ϵ)
end
...
# ╔═╡ 655f7b47-66c4-4654-ba52-e1e53821b7aa
function SEIR(du, u, p, t)
	b, s, g = p
	du[1] = -b * u[1] * u[3]
	du[2] = b * u[1] * u[3] - g * u[2]
	du[3] = g * u[2] - s * u[3]
	du[4] = s * u[3]
end
...
# ╔═╡ ff3aebae-8f23-4d68-8821-fa5a62bbb2d1
function SEIRS(du, u, p, t)
	b, s, g, d = p
	du[1] = -b * u[1] * u[3] + d * u[4]
	du[2] = b * u[1] * u[3] - g * u[2]
	du[3] = g * u[2] - s * u[3]
	du[4] = s * u[3] - d * u[4]
end
...
# ╔═╡ 35d1a2f2-aa08-4ebb-a228-8e88eb3facfa
function costo(sol, t_, dato_ola, ind)
	if ind == 2
		return sum((sol.(t_, idxs=1) .* sol.(t_, idxs=ind) .* sol.prob.p[1] - dato_ola).^2)
	elseif ind == 3
		return sum((sol.(t_, idxs=2) .* sol.prob.p[3] - dato_ola).^2)
	end
end
...
md""" ### Comienza la lucha con el sir


"""

# ╔═╡ e7d33b07-80a4-41b4-abe4-c546f18aaa26
begin
	prob_SIR = ODEProblem(SIR,[0.999, 1 - 0.999, 0.],(1,tf_ola1),abstol=1e-14,reltol=1e-10)
end

# ╔═╡ d77992e2-343e-44f3-9d13-8f5b67a378c0
begin
	func_SIR =
		build_loss_objective(prob_SIR,AutoTsit5(Rosenbrock23()),sol -> costo(sol, t_1, ola1, 2), Optimization.AutoFiniteDiff(),prob_generator = (SIR_problem_,q)->remake(SIR_problem_,u0=[1 - q[1],q[1],  0.],p = q[2:3]))
end

# ╔═╡ 088def1a-aa34-4cd2-849a-ae3cf071db42
begin
	semilla_optprob_SIR  = [1e-7, 1.8, 1.8] #S_0 β σ
	
	optprob_SIR = OptimizationProblem(func_SIR, semilla_optprob_SIR, lb=[0., 0.3, 0.4], ub=[1.,10.,10.])
end

# ╔═╡ 339ae58c-b21b-44ef-9a86-4c679d0e74a8
begin
	sol_param_SIR = solve(optprob_SIR,SAMIN(),maxiters=100_000)
end

# ╔═╡ 34dae254-4bab-4524-9729-45fc4686dfea
begin
sol_SIR  = 
	solve(remake(prob_SIR,u0 = [1 - sol_param_SIR[1],sol_param_SIR[1],  0.] , p = sol_param_SIR[2:3]), AutoTsit5(Rosenbrock23()), abstol=1e-14,reltol=1e-10)
end

# ╔═╡ a22bc379-4e79-49e1-9681-c6cea97aef7f
md""" Graficamos primero la evolución de las 3 poblaciones


"""

# ╔═╡ 60c3e8de-a950-4695-82d6-300f9c04fff9
begin
plot(sol_SIR[1,:], label = "S")
plot!(sol_SIR[2,:], label = "I") 
plot!(sol_SIR[3,:], label = "R")
end

# ╔═╡ 24d1129b-0912-4ae5-93c8-ce7d63c766e7
md""" Ahora si comparamos los casos con los datos reales


"""

# ╔═╡ 80a4c33a-e1b0-443c-87f8-9c3366e30901
begin
	plot( sol_SIR.(t_1, idxs=1) .* sol_SIR(t_1, idxs=2) .* sol_param_SIR[2], label = "Nuestro modelo")
	scatter!(ola1, label = "Datos reales")
	title!("SIR")
end

# ╔═╡ ea5cca4a-476e-45ca-a56b-23d5e89faa9e
md""" ### Continua la lucha con el seir


"""

# ╔═╡ d4868971-77a2-482b-8f33-47724d9b250f
begin
	...
	σSeir = 6
	γSeir = 4.3
	semilla_optprob_SEIR = [S₀Seir, ρ₀Seir, βSeir, σSeir, γSeir]
	prob_SEIR = ODEProblem(SEIR, [S₀Seir, ρ₀Seir * (1 - S₀Seir), 1 - S₀Seir - (ρ₀Seir * (1 - S₀Seir)), 0.], (1,tf_ola1), abstol=1e-14, reltol=1e-10)
end

# ╔═╡ 676d577f-c672-4fdb-a66f-b9f3d05e8346
begin
	func_SEIR    =
		build_loss_objective(prob_SEIR,AutoTsit5(Rosenbrock23()),sol -> costo(sol, t_1, ola1, 3), Optimization.AutoFiniteDiff(),prob_generator = (prob_SEIR,q)-> remake(prob_SEIR, u0 = [q[1], q[2]*(1 - q[1]), 1 - q[1] - (q[2]*(1 - q[1])), 0.], p = q[3:5]))
end

# ╔═╡ 98e8d072-4fd8-4fca-b7ac-8fa19f99059d
begin
	optprob_SEIR = OptimizationProblem(func_SEIR, semilla_optprob_SEIR, lb=[0.99, 0., 5, 5, 4.],ub=[1. ,1. ,10. ,10., 15.])
end

# ╔═╡ 4bc58baa-5e0f-494e-9d81-5b7062cb13a6
begin
	sol_param_SEIR = solve(optprob_SEIR,SAMIN(),maxiters=400_000)
end

# ╔═╡ c40e772b-1e92-4e5c-8a78-39e131dcbfe5
begin
	sol_SEIR  = solve(remake(prob_SEIR,u0 = [sol_param_SEIR[1], sol_param_SEIR[2] * (1 - sol_param_SEIR[1]), 1 - sol_param_SEIR[1] - (sol_param_SEIR[2] * (1 - sol_param_SEIR[1])), 0.], p = sol_param_SEIR[3:5]), AutoTsit5(Rosenbrock23()), abstol=1e-14,reltol=1e-10)
end

# ╔═╡ 28cd051d-db34-4022-8c2b-798d0d55cff8
plot(sol_SEIR)# poner lindo

# ╔═╡ a57b339b-692b-4b4c-9dbc-9292097449c2
md"""  Comparamos con los datos reales y observamos un modelo mas acertado (aunque esto no se ve siempre por la aleatoriedad del samin) """

# ╔═╡ 5ed90642-5369-46b1-aa6d-009cbda4b066
begin
	plot( sol_SEIR.(t_1, idxs=1) .* sol_SEIR(t_1, idxs=3) .* sol_param_SEIR[3], xlims= (1,tf_ola1) , label = "Nuestro Modelo")
	scatter!(ola1, label = "Datos Reales")
	title!("SEIR")
end

# ╔═╡ 4054d03f-c00d-48da-9496-549c99dc06bc
md""" ### Seguimos la lucha con el seirs


"""

# ╔═╡ e7af3eba-6a65-4e48-bbcd-6f24ed0e7493
begin
	S₀Seirs = 0.99
	ρ₀Seirs = 0.2 			     
	βSeirs = 6
	...
	semilla_optprob_SEIRS = [S₀Seirs, ρ₀Seirs, βSeirs, σSeirs, γSeirs, δSeirs]
	
	prob_SEIRS = ODEProblem(SEIRS, [S₀Seirs, ρ₀Seirs*(1 - S₀Seirs), 1 - S₀Seirs - (ρ₀Seirs*(1 - S₀Seirs)), 0.], (1,tf_ola1), abstol=1e-14, reltol=1e-10)
end

# ╔═╡ ccf99ea1-bc09-4a63-a726-a50709217291
begin
	func_SEIRS =
		build_loss_objective(prob_SEIRS, AutoTsit5(Rosenbrock23()), sol -> costo(sol, t_1, ola1, 3), Optimization.AutoFiniteDiff(), prob_generator = (prob_SEIRS, q)->remake(prob_SEIRS, u0 = [q[1], q[2]*(1 - q[1]), 1 - q[1] - (q[2]*(1 - q[1])), 0.], p = q[3:6]))
end

# ╔═╡ cf60e662-89da-4d09-beaf-f2ddb2228b74
begin
	optprob_SEIRS = OptimizationProblem(func_SEIRS,semilla_optprob_SEIRS, lb=[0.99, 0., 5, 5, 4., 0.],ub=[1. ,1. ,10. ,10., 15., 0.5])
	
	sol_param_SEIRS = solve(optprob_SEIRS,SAMIN(),maxiters=400_000)
end

# ╔═╡ ec3e2998-f9d2-4042-b361-825b2b116fee
begin
	sol_SEIRS  = 
		solve(remake(prob_SEIRS,u0 = [sol_param_SEIRS[1], sol_param_SEIRS[2]*(1 - sol_param_SEIRS[1]), 1 - sol_param_SEIRS[1] - (sol_param_SEIRS[2]*(1 - sol_param_SEIRS[1])), 0.] , p = sol_param_SEIRS[3:6]), AutoTsit5(Rosenbrock23()), abstol=1e-14,reltol=1e-10)
end

# ╔═╡ 61cd06bc-cc9f-49f4-90ed-c633f4540e01
plot(sol_SEIRS) 

# ╔═╡ 95a73813-5fb5-4572-b8e7-59c33e624932
md""" Volvemos a aclarar que por la aleatoriedad del samin en cada ejecutada de las funciones pueden variar un poco los graficos que se ven. Sin embargo con estos parámetros iniciales y estos límites inferiores y superiores para los parámetros encontramos los resultados esperados tras pocos intentos.


"""

# ╔═╡ 9d5dd97b-8ac3-4623-af17-9c41fa8e6438
begin
	plot( sol_SEIRS.(t_1, idxs=1) .* sol_SEIRS(t_1, idxs=3) .* sol_param_SEIRS[3], xlims = (1,41), label = "Nuestro Modelo")
	scatter!(ola1, label = "Datos Reales")
	title!("SEIRS")
end
...
begin
	...
	δSeirs2 = 0.1
					#PRUEBEN VALORES
	semilla_optprob_SEIRS2 = [S₀Seirs2, ρ₀Seirs2, βSeirs2, σSeirs2, γSeirs2, δSeirs2]
	
	prob_SEIRS2 = ODEProblem(SEIRS, [S₀Seirs2, ρ₀Seirs2*(1 - S₀Seirs2), 1 - S₀Seirs2 - (ρ₀Seirs2*(1 - S₀Seirs2)), 0.], (1,tf_ola2), abstol=1e-14, reltol=1e-10)
end

# ╔═╡ f5192396-c833-4baa-b763-a1f857524fde
begin
	func_SEIRS2 =
		build_loss_objective(prob_SEIRS2, AutoTsit5(Rosenbrock23()), sol -> costo(sol, t_2, ola1y2, 3), Optimization.AutoFiniteDiff(), prob_generator = (prob_SEIRS2, q)->remake(prob_SEIRS2, u0 = [q[1], q[2]*(1 - q[1]), 1 - q[1] - (q[2]*(1 - q[1])), 0.], p = q[3:6]))
end

# ╔═╡ d11901dc-82a9-4b25-a2b8-4f2cac8651ff
begin
	optprob_SEIRS2 = OptimizationProblem(func_SEIRS2, semilla_optprob_SEIRS2, lb=[0.99, 0., 4, 4, 4., 0.], ub=[1. ,1. ,10. ,10., 15., 0.5])
	
	sol_param_SEIRS2 = solve(optprob_SEIRS2,SAMIN(),maxiters=50_000);
end

# ╔═╡ 172813b1-23be-4087-8341-6d4bb512ede0
begin
	sol_SEIRS2  = 
		solve(remake(prob_SEIRS2, u0 = [sol_param_SEIRS2[1], sol_param_SEIRS2[2]*(1 - sol_param_SEIRS2[1]), 1 - sol_param_SEIRS2[1] - (sol_param_SEIRS2[2]*(1 - sol_param_SEIRS2[1])), 0.] , p = sol_param_SEIRS2[3:6]), AutoTsit5(Rosenbrock23()), abstol=1e-14,reltol=1e-10);
end

# ╔═╡ a8b23960-79f1-4f1f-afd7-45ec5cf174cd
md""" De alguna forma observamos como la curva azul no replica precisamente la nueva subida en la segunda ola, pero que si refleja el comportamiento "promedio" durante esta.


"""

# ╔═╡ a2d3acc4-18de-47c0-ae86-067e7c501b80
begin
	plot( sol_SEIRS2.(t_2, idxs=1) .* sol_SEIRS2(t_2, idxs=3) .* sol_param_SEIRS2[3], xlims = (1,51), label = "Nuestro Modelo")
	scatter!(ola1y2, label = "Datos Reales")
	title!("SEIRS")
end

# ╔═╡ b5da7e62-c00a-4385-ac4e-dd8a48bf2193
md""" ### Segunda Parte """

# ╔═╡ 908af2ce-0149-4fd8-a018-f26976c232f5
md""" #### Incluir mortalidad. 
...
# ╔═╡ 277d9a9e-d123-4253-a3ac-ba627c4d2a37
function SEIR_Mortalidad(du, u, p, t)
	s, b, g, ϵ, nat ,mort = p
	N = u[1]+u[2]+u[3]+u[4]
	du[1] =nat*N -b*u[1]*u[3] / N - mort * u[1]
	
	du[2] = b*u[1]*u[3] / N - g * u[2]- mort * u[2]
	
	du[3] = g * u[2] - s * u[3]- mort * u[3] - ϵ * u[3]
	
	du[4] = s * u[3]- mort * u[4]
	
end

# ╔═╡ 52276376-44d8-48bc-b372-80efda9283dd
function SIR_Mortalidad(du, u, p, t)
	s, b, ϵ, nat ,mort = p
	N = sum(u)
	du[1] = nat*N -b*u[1]*u[2]/N - mort*u[1]
	
	du[2] = b*u[1]*u[2]/N - s*u[2] - ϵ*u[2] -mort*u[2]
	
	du[3] = s*u[2] - mort*u[3]
end

# ╔═╡ ec301b06-2012-4808-acf6-9de1969fa018
md""" ### Funciones del Sir"""

# ╔═╡ 5d3478a7-9688-4e88-bf82-1095b26b2266
begin
	prob_SIR_Mortalidad = ODEProblem(SIR_Mortalidad,[0.999, 1 - 0.999, 0.],(1,tf_ola1),abstol=1e-14,reltol=1e-10)
end

# ╔═╡ 0f31be8f-2a41-45af-b4e6-181acce2d136
begin
	func_SIR_Mortalidad =
		build_loss_objective(prob_SIR_Mortalidad,AutoTsit5(Rosenbrock23()),sol -> costo(sol, t_1, ola1, 2), Optimization.AutoFiniteDiff(),prob_generator = (SIR_problem_Mortalidad,q)->remake(SIR_problem_Mortalidad,u0=[1 - q[1],q[1],  0.],p = q[2:6]))
end

# ╔═╡ 42296773-aec3-4b93-abdd-3a8d3215c82a
begin
	semilla_optprob_SIR_Mortalidad = [0.0015546,0.12545,0.1113,0.24235,0.54631,0.63]# p =s0, sig, b , ϵ, nat ,mort
	
	optprob_SIR_Mortalidad = OptimizationProblem(func_SIR_Mortalidad, semilla_optprob_SIR_Mortalidad, lb=zeros(6), ub=ones(6))
end

# ╔═╡ 3f597d2f-8f5c-4fee-b91f-eab748fcfd9d
begin
	sol_param_SIR_Mortalidad = solve(optprob_SIR_Mortalidad,SAMIN(),maxiters=50_000)
end

# ╔═╡ a7579068-d71e-41ca-8d32-11daf6272560
begin
sol_SIR_Mortalidad  = 
	solve(remake(prob_SIR_Mortalidad,u0 = [1 - sol_param_SIR_Mortalidad[1],sol_param_SIR_Mortalidad[1],  0.] , p = sol_param_SIR_Mortalidad[2:6]), AutoTsit5(Rosenbrock23()), abstol=1e-14,reltol=1e-10)
end

# ╔═╡ 9e320e3b-08d4-49a9-a0b1-bbb4c7980a02
md""" Vemos como mejora notablemente la precisión del modelo al incluir la mortalidad


"""

# ╔═╡ 575f534b-3428-4934-965d-46c0a8102472
begin
	plot( sol_SIR_Mortalidad.(t_1, idxs=1) .* sol_SIR_Mortalidad(t_1, idxs=2) .* sol_param_SIR_Mortalidad[2], label = "Nuevo modelo")
	scatter!(ola1, label = "Datos reales")
	
	plot!( sol_SIR.(t_1, idxs=1) .* sol_SIR(t_1, idxs=2) .* sol_param_SIR[2], label = "Viejo modelo")
	title!("SIR Mortalidad")
end

# ╔═╡ 5d6fc5b2-0645-4ded-bf3b-a4add7f9f04a
md""" ### Funciones del Seir"""

# ╔═╡ 3157dfbf-bc6a-437c-ace9-994f6ae9aedf
begin	     
		S₀SeirM =  1-1e-7
		ρ₀SeirM = 0.05 	
		σSeirM = 2.5
		...
		mortM = 1e-1
        #s, b, g, ϵ, nat ,mort = p		
       semilla_optprob_SEIR_M = [S₀SeirM,ρ₀SeirM,σSeirM,βSeirM,γSeirM,ϵM,natM,mortM]	
	end

# ╔═╡ 35371e22-0024-4229-810c-08913797c191
begin
		prob_SEIR_M = ODEProblem(SEIR_Mortalidad, [S₀SeirM, ρ₀SeirM * (1 - S₀SeirM), 1 - S₀SeirM - (ρ₀SeirM * (1 - S₀SeirM)), 0.], (1,tf_ola1), abstol=1e-14, reltol=1e-10)
end

# ╔═╡ 02439c21-d719-48d7-8dbb-96305914e1ba
begin
		func_SEIR_M    =
			build_loss_objective(prob_SEIR_M,AutoTsit5(Rosenbrock23()),sol -> costo(sol, t_1, ola1, 3), Optimization.AutoFiniteDiff(),prob_generator = (prob_SEIR_M,q)-> remake(prob_SEIR_M, u0 = [q[1], q[2]*(1 - q[1]), 1 - q[1] - (q[2]*(1 - q[1])), 0.], p = q[3:8]))
	end

# ╔═╡ dc73185f-d4e6-4452-a212-10acf8ce5c2d
begin
	optprob_SEIR_M = OptimizationProblem(func_SEIR_M, semilla_optprob_SEIR_M, lb=[0.,0.,2.,2.,0.,0.,0.,0.],ub=[1.,1.,10.,10.,20.,3.,1.,0.1])
end

# ╔═╡ fe76b2e3-77d3-4f9e-a46e-ae7f4f9ae210
begin
		sol_param_SEIR_M = solve(optprob_SEIR_M,SAMIN(),maxiters=100_000)
end

# ╔═╡ ad96e4dd-32f0-4216-92df-a160c7cccb54
begin
		sol_SEIR_M  = solve(remake(prob_SEIR_M,u0 = [sol_param_SEIR_M[1], sol_param_SEIR_M[2] * (1 - sol_param_SEIR_M[1]), 1 - sol_param_SEIR_M[1] - (sol_param_SEIR_M[2] * (1 - sol_param_SEIR_M[1])), 0.], p = sol_param_SEIR_M[3:8]), AutoTsit5(Rosenbrock23()), abstol=1e-14,reltol=1e-10)
	end

# ╔═╡ ad4a1267-fda5-4e74-8f57-4f70e14e0d32
md""" Vemos como mejora el modelo al incluir la mortalidad"""

# ╔═╡ b41991f5-1284-4654-bfc6-73c88b315100
begin
	plot(sol_SEIR.(t_1, idxs=2) .* sol_param_SEIR[5], xlims = (1,tf_ola1), label = "Viejo Modelo")
	plot!(sol_SEIR_M(t_1, idxs=2) .* sol_param_SEIR_M[5], xlims = (1,tf_ola1), label = "Nuevo Modelo")
	scatter!(ola1, label = "Datos Reales")
	title!("SEIR")
end

# ╔═╡ 0fa95950-acd6-43f9-8922-f909aca53a78
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "4ae3823fa26b08851a50f93b01cdbbaf4b5c16c6"
